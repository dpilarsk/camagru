...
<div class="container">
	<div class="preview">
		<div class="main">
			...
			<div class="opinion">
				<div class="like" id="like" style="display: inline;<?php if (isset($_SESSION['token']) && $likes->getLikeUser($_SESSION['token'], $_GET['id'])){ ?> border: solid 1px blue;<?php } ?>">
			...
			<?php if (isset($_SESSION['id']) && $_SESSION['id'] == $picture['user_id']){ ?>
		...
		<div class="side">
			...
			<form action="#" id="comment">
				...
				<input type="text" value="<?php if (isset($_SESSION['token'])) echo $_SESSION['token']; ?>" name="token" style="display: none;">
...
<script>
	...
	form.addEventListener("submit", function (e) {
		...
		data.append('token', "<?php if (isset($_SESSION['token'])) echo $_SESSION['token']; ?>")
		data.append('picture_id', "<?php if (isset($_GET['id'])) echo $_GET['id']; ?>")
		document.getElementById('submit').disabled = true
	...
	document.getElementById('like').addEventListener('click', function (e) {
		...
		data1.append('token', "<?php if (isset($_SESSION['token'])) echo $_SESSION['token']; ?>")
		data1.append('picture_id', "<?php if (isset($_GET['id'])) echo $_GET['id']; ?>")
		xhr1.open('POST', '/functions/like.php')
		xhr1.send(data1)
	})
	xhr1.onreadystatechange = function () {
		if (xhr1.readyState === 4 && xhr1.status === 200) {
			...
			{
				document.getElementById('like').style.border = ''
				document.getElementById('likeNum').innerHTML = num > 0 ? num - 1 : num
			}
		}
	}
</script>
